...
import { z } from 'zod';
import { createToken, requireAdmin, requireAuth, type AuthRequest } from './auth.js';
...
function validationMessage(error: z.ZodError) {
  return error.issues.map(issue => {
    ...
    if (field === 'password' && issue.code === 'too_small') return 'Mật khẩu phải có ít nhất 8 ký tự';
    if (field === 'email') return 'Email không hợp lệ';
    if (field === 'fullName' && issue.code === 'too_small') return 'Họ tên phải có ít nhất 2 ký tự';
...
async function ensureAdmin() {
  ...
  await query(
    `INSERT INTO users (email, password_hash, full_name, role)
     VALUES ($1, $2, $3, 'admin')`,
    [email.toLowerCase(), hash, 'Lab Administrator'],
  );
}
...
app.post('/api/auth/login', async (req, res) => {
  ...
  const result = await query<{
    id: string; email: string; password_hash: string; full_name: string; role: 'admin' | 'member'; status: string;
  }>('SELECT id, email, password_hash, full_name, role, status FROM users WHERE email = $1', [parsed.data.email.toLowerCase()]);
...
app.post('/api/members', requireAuth, requireAdmin, async (req, res) => {
  ...
  try {
    const result = await query(
      `INSERT INTO users (email, password_hash, full_name, role, student_code, phone, status)
       VALUES ($1, $2, $3, $4, $5, $6, $7)
       RETURNING ${userSelect}`,
      [data.email.toLowerCase(), hash, data.fullName, data.role, data.studentCode, data.phone, data.status],
    );
...
app.patch('/api/members/:id', requireAuth, requireAdmin, async (req, res) => {
  ...
  if (!parsed.success) return res.status(400).json({ error: validationMessage(parsed.error) });
  const data = parsed.data;
  const current = await query<any>('SELECT * FROM users WHERE id = $1', [req.params.id]);
  if (!current.rows[0]) return res.status(404).json({ error: 'Member not found' });
  const old = current.rows[0];
  const hash = data.password ? await bcrypt.hash(data.password, 12) : old.password_hash;
  const result = await query(
    `UPDATE users SET email=$1, password_hash=$2, full_name=$3, role=$4, student_code=$5,
...
app.delete('/api/members/:id', requireAuth, requireAdmin, async (req: AuthRequest, res) => {
...
app.get('/api/circuits', requireAuth, async (req: AuthRequest, res) => {
  const result = await query(
    `SELECT id, name, owner_id AS "ownerId", project_id AS "projectId", version,
     created_at AS "createdAt", updated_at AS "updatedAt"
     FROM circuits WHERE owner_id = $1 ORDER BY updated_at DESC`,
    [req.user!.id],
  );
  res.json(result.rows);
});

app.post('/api/circuits', requireAuth, async (req: AuthRequest, res) => {
...
app.get('/api/circuits/:id', requireAuth, async (req: AuthRequest, res) => {
  const result = await query(
    `SELECT id, name, owner_id AS "ownerId", project_id AS "projectId", schematic,
     sim_config AS "simConfig", version, created_at AS "createdAt", updated_at AS "updatedAt"
     FROM circuits WHERE id = $1 AND owner_id = $2`,
    [req.params.id, req.user!.id],
  );
  if (!result.rows[0]) return res.status(404).json({ error: 'Circuit not found' });
  res.json(result.rows[0]);
});

app.put('/api/circuits/:id', requireAuth, async (req: AuthRequest, res) => {
  const parsed = z.object({
    name: z.string().min(1),
    projectId: z.string().uuid().optional().nullable(),
    schematic: z.unknown(),
...
app.delete('/api/circuits/:id', requireAuth, async (req: AuthRequest, res) => {
  const result = await query('DELETE FROM circuits WHERE id=$1 AND owner_id=$2 RETURNING id', [req.params.id, req.user!.id]);
  if (!result.rowCount) return res.status(404).json({ error: 'Circuit not found' });
  res.status(204).end();
});
